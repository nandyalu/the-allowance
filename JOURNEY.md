...
**2026-09-10 — the agent is asked when the market is shut, instead of being turned away at the door.** The prompt has invited it to wake at any hour since 2026-09-05: "You may ask for any time, including before the open, after the close and at the weekend. Research and planning work at any hour... Waking early to commission the analyses you want ready for the open is a good use of this." None of that was true. `run_once` returned before the model was asked whenever `is_us_market_hours()` was false, so a wakeup the agent had scheduled for 6am to prepare the open produced nothing at all — no research, no exit adjustment, not even a note.

**Found from a screenshot of a fresh deployment**, whose only decision card read "The US market is closed, so no order could be placed. The agent decides automatically each weekday at 13:35 UTC, five minutes after the open." Two faults in one sentence: the pass had been refused, and the message named a fixed 13:35 pass that was itself removed on 2026-09-05. The scheduler's own comment says "**The market-hours gate is gone**", and it was — from the scheduler. A second copy of it survived one call deeper, where nobody looked.

**The gate was answering the wrong question.** It exists because Webull rejects an order outside the session, and a decision made on a shut market's prices is stale by the next open. Both true, and both are arguments about *orders* — not about whether the agent may think. Research, moving a stop, untracking a name, leaving a note and choosing the next wakeup all work perfectly well at midnight, and the first line of every prompt already says whether the market is open, closed for the day, or closed for the weekend. The agent was being protected from a mistake it had been told how to avoid.

**So the refusal moves to where it belongs: the order.** `sandbox_broker` already refuses one outside the session and records it as a broker failure, and the last five failures appear in the next prompt. That is the loop the rules already describe — "the broker rejects one outright while the market is shut, and you will see that rejection here next time" — and it is now the loop that actually runs.

**This is a restriction that existed only because nobody had built the tool yet**, which CLAUDE.md calls a gap rather than a rule. The tool — the agent's own clock — was built on 2026-09-05, and this is the last thing that was still ignoring it.

**2026-09-10 — the setup instructions were audited against the code, and three of them could not have worked.** The first-run page landed the same day, so the obvious next question was whether the written instructions still matched. They did not, and the three worst faults were all the same shape: text that reads as correct and sends a person somewhere that does not exist.

**The setup page told people to paste a variable the app never reads.** Its advice for a hosted model endpoint was `LLM_BACKEND_URL=…`. The real name is `TRADINGAGENTS_LLM_BACKEND_URL`, and nothing maps the short one. So the one feature built to stop people guessing at variable names was itself guessing, and a person who followed it exactly would watch the endpoint check keep failing with no way to tell why. It shipped in the image running here and the audit caught it before the code was committed, so the page reaches git already printing the names the app reads, with no reliance on a compose file to translate them.

**The repo told people to deploy with a file the repo does not contain.** `dockge/` is in `.gitignore` — it is this machine's working directory, not a shipped artifact — while `README.md` and two docs pages each called `dockge/trading-experiment.compose.yaml` "the working template". A clone has no such file. `compose.example.yaml` is now tracked and is what those pages point at.

**`.env.example` still described the app as it was in July.** It offered `DISCORD_BOT_TOKEN` and `DISCORD_CHANNEL_ID` for a bot deleted on 2026-09-01, marked `WEBULL_SANDBOX` optional when it is the boundary the whole experiment rests on, named a model retired on 2026-09-01, and listed none of `WEBULL_ACCOUNT_ID`, `WEBULL_ACCOUNT_CLASS`, `AGENT_BUDGET`, `RESEARCH_PRICE_USD` or `FRED_API_KEY`. A file that is wrong in that many places is worse than no file, because a reader has no way to tell which lines to trust.

**`WEBULL_ACCOUNT_ID` appeared in no document at all.** It became a hard requirement on 2026-09-09 — the fourth guard, with no default, deliberately — and the pages that list every required variable were written before it existed. Following them exactly produced a container that starts, reports healthy, and places no order ever.

**The pattern is worth naming, because it will recur.** Every one of these was introduced by a change that was correct in the code and left the instructions behind, and none of them fails loudly. A wrong variable name, a missing file, a stale example: all three read as authoritative and all three cost a self-hoster an afternoon. The check that catches them is reading the docs against the code rather than against memory, which is what this entry records having done.

**A second pass over the same pages found nine more, which says the first pass was not thorough enough rather than that the docs decayed again in an hour.** The first pass chased what today's changes had touched. The second checked every remaining factual claim against the code that implements it, and that is the difference — the faults below were all months old and none of them was near anything edited this week.

**The most wrong was a time.** `README.md` scheduled the final pass of the day at "20:55" under a column headed "Time (UTC)". The code puts it at 15:55 *Eastern*, and says in its own comment that it is decided against the real close "so it does not drift by an hour twice a year the way the other jobs here do". So the table was correct in winter and an hour out for the rest of the year, including today. `docs/overview.md` had it right, which is the useful part of the finding: two pages describing one schedule will disagree eventually, and the one that reads as more authoritative is not reliably the correct one.

**Three references to slash commands survived the removal of all twenty-three.** `docs/overview.md` still told a reader to set the trade horizon with `/horizon`, the model with `/model`, and to view candidates with `/candidates`. `docs/discord.md` states plainly on its fifth line that no slash commands exist, so the docs contradicted each other rather than merely being stale.
...
**It exists because a second container is about to run.** The Cerebras experiment gets its own deployment, and the sandbox host exposes five accounts across two equities classes. The existing guards narrow that to "an `INDIVIDUAL_CASH` account whose number starts with `DE`" — which is exactly one account today and would be exactly one account for a second container too. **Two deployments resolving the same rule would resolve to the same book**, and the record would afterwards be unable to say which container placed which order. That is the same failure the manual-controls removal was about, arriving from a different direction: not a second decision-maker, but two decision-makers with one ledger between them.

**Fail closed, and that is a deliberate operational cost.** An unset variable stops order flow rather than falling back to the old behaviour, so the running deployment will not trade after its next restart until the variable is set. A default would defeat the point — the whole value here is that a person had to write down which book this container owns, and a guard that guesses when nobody wrote anything down is not a guard.

**It matches either the account number or the internal id**, whichever the operator pasted. Those two are different shapes (`DEL546C9` against a numeric id), so accepting both cannot make a wrong value look right — a value that matches neither matches nothing, and the app refuses. Requiring the *right* one of two identifiers, with no way to tell from the console which the app wanted, would only produce confusing refusals.

**It narrows; it never widens.** `_assert_sandbox()`, the `DE` prefix check and the account-class check all still run, and this is applied after them: an account has to pass all three and then be the named one. Nothing here can be used to reach an account the previous three would have refused, which is the property that must hold for any future edit to this file.

**2026-09-09 — LLM calls back off when a vendor says they are too fast, which the app could not do at all.** Found while testing whether Cerebras' free tier could run a real analysis: the console showed a mix of 200s and 429s, on both the requests-per-minute and the tokens-per-minute ceilings. The app had no answer to either. LangChain's client retries a 429 on its own, but each analysis retries **independently** — so seven concurrent analyses discover the same limit seven times over and keep the endpoint saturated while doing it.

**It is reactive on purpose, and that is what makes it free for the local pool.** A fixed pace would slow every ollama call for nothing: a GPU this app owns has no rate limit and never returns 429. A throttle that engages only when a vendor actually refuses costs exactly zero on the local path, and self-tunes on a metered one without anybody hardcoding that vendor's published limits — which, as with Webull's market-data endpoint earlier today, are frequently not published at all.

**One pace, shared across every concurrent analysis**, for the reason the per-analysis retry fails: the limit belongs to the account, not to the run. When one analysis is told to slow down, the other six are the reason it was told, and they have to hear it too.

**The seam is `llm.client.create`, not the vendored graph.** Every call TradingAgents makes eventually reaches that method on the two shared client objects, so wrapping it there covers the whole run — the analysts, the debate, the trader — without editing the submodule. It is the same seam the thinking capture uses, and the throttle is attached in `_build_graph` beside the usage tracker and the trace recorder, which are already attached that way for the same reason.

**Two things this deliberately does not do.** It does not pace proactively, so a metered vendor still pays one 429 to teach it the limit — self-tuning is worth more than a guess at somebody's ceiling. And it does not make token-per-minute limits predictable: the app cannot know a call's token count before sending it, so a TPM refusal is handled the same way as an RPM one, by backing off after the fact rather than before.
